...
_loop1:
	...
	mov di,600h + (8*8);
	mov word [di], _prot_icall8
	mov di,600h + (0x0d*8)
	mov word [di], _prot_icalld
	mov di,600h + (0x0e*8)
	mov word [di], _prot_icall_e
	;sti
	...
	xor eax,eax
	rep stosd
	mov eax, 0x202000 + 0x07
	mov dword [0x200000], eax
	mov ecx,1024
	mov edi,0x202000
	mov ebx,0
_loop_page1:
	...
	mov eax,0x204000 + 0x07;
	mov edi,0x90000000
	shr edi,22
	mov [edi*4 + 0x200000],eax; write page directory of page index
	mov edi,0x204000
	mov ecx,1024
	xor eax,eax
	rep stosd
	mov eax,0xb8000 + 0x06
	mov [0x204000],eax
	...
	mov byte [0xb8000], al
	mov al,'Z'
	mov byte [0x90000000+2], al
	mov al,'K'
	mov byte [0x90000000+4], al
	jmp $
...
_prot_icalld:
	...
	call _put_ebx
	mov al,'D'
	mov byte [0xb8000+20], al
	iretd

_put_ebx:
	mov al,0x0a
	call _putc
	mov dword [_paddings_],ebx
	mov eax,_paddings_
	mov ecx, 4
	call _put_mem
	ret
...
_prot_icall_e:
	...
	pusha
	mov al,'E'
	mov byte [0xb8000+22], al
	call _put_ebx
	mov eax,0xb8000 + 0x07
	mov [0x204000],eax
	popa
	iretd
...
_tss_dt:
	...
	_tss_dt_base_m    db 0
	_tss_dt_attrib_lo db 0x89; P DPL S TYPE
...
_idtx:
	_idtx_offset_lo dw _prot_icall;  // _prot_icall8/_prot_icalld
	_idtx_selector  dw (_gdt1 - _gdt0)
	_idtx_attrib_lo db 0; DWORD COUNT,not used
	_idtx_attrib_hi db 0xEE; P DPL S TYPE
